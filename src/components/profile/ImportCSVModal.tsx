...
const ImportCSVModal = ({ isOpen, onClose, onSuccess }: ImportCSVModalProps) => {
  ...
  const handleUpload = async () => {
    ...
    if (!token) {
      toast.error("Veuillez vous connecter pour importer des profils");
      return;
    }
    ...
    try {
      ...
      await axios.post("http://localhost:5000/api/profiles/import_csv", formData, {
        headers: {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Importer des profils depuis un fichier CSV</DialogTitle>
          <DialogDescription>
            Téléchargez un fichier CSV contenant les profils à importer dans le système.
            Le fichier doit contenir les colonnes suivantes: Profil, Lieu_de_recherche, Disponibilité, Points_forts, Expérience, Compétence, Contrat, Metier_regroupe.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="sm:justify-between">
          ...
          >
            {isUploading ? "Importation en cours..." : "Importer les profils"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
