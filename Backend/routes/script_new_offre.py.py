...
for i in range(similarity_total.shape[0]):
    for j in range(similarity_total.shape[1]):
        offre_id = df_offres.iloc[j]['offre_id']
        candidat_id = df_candidats.iloc[i]['candidat_id']
        all_matches.append((candidat_id, offre_id, similarity_total[i, j]))
...
client = MongoClient("mongodb://localhost:27017")
db = client["matchrh"]
collection = db["OffresMeuilleurCandidat"]

offres_meilleurs_candidats_clean = offres_meilleurs_candidats.where(pd.notnull(offres_meilleurs_candidats), None)

documents = offres_meilleurs_candidats_clean.to_dict(orient="records")
